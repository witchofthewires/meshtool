## meshtool - meshtastic CLI Project

### Installation

#### Windows
To install via Make:
```
git clone https://github.com/witchofthewires/meshtool.git
cd meshtool
make winstall
```
To manually install:
```
git clone https://github.com/witchofthewires/meshtool.git
cd meshtool
python -m venv venv
.\venv\Scripts\activate
py -m pip install -r requirements.txt
py -m pip install .
```
To run associated tests:
```
py -m pytest -v
make test # on systems with Make installed, executes 'python -m pytest -v'
py -m pytest -v -m "not slow" # do not run tests which take 5+ seconds
py -m pytest -v -m "radio" # only run tests which require a serial connection to a Meshtastic radio
py -m coverage run -m --source=meshtool pytest # check test coverage
py -m coverage report # view coverage report
py -m coverage html # create html report at 'htmlcov\index.html'
```

#### Linux
To install:
```
git clone https://github.com/witchofthewires/meshtool.git
cd meshtool
make install
```
To run associated tests:
```
make test
```